...
/**
 * Provides a derniers articles block.
 *
 * @Block(
 *   id = "test_module_derniers_articles",
 *   admin_label = @Translation("Derniers Articles"),
 *   category = @Translation("Custom")
 * )
 */
class DerniersArticlesBlock extends BlockBase implements ContainerFactoryPluginInterface
{
  ...
  public function blockForm($form, FormStateInterface $form_state)
  {
    ...
    $form['block_cache'] = [
      '#type' => 'number',
      '#title' => $this->t('Block cache invalidation interval in seconds'),
      '#default_value' => $this->configuration['block_cache'],
      '#options' => array_combine($range, $range),
    ];
  ...
  public function blockSubmit($form, FormStateInterface $form_state)
  {
    ...
    $this->configuration['block_cache'] = $form_state->getValue('block_cache');
  }
  ...
  public function build()
  {
    ...
    if ($feed = $this->feedStorage->load($this->configuration['feed'])) {
      ...
      if ($result) {
        ...
        $build['#cache']['max-age'] = $this->configuration['block_cache'];
